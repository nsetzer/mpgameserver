...
from .logger import mplogger, PeerLogger
...
class PacketType(SerializableEnum):
    ...
    The CLIENT_HELLO, SERVER_HELLO, CHALLENGE_RESP types implement a
    three way handshake that agree on an encryption key and verify both
    sides are implementing the same protocol.

    :attr UNKNOWN:        not used
    :attr CLIENT_HELLO:   sent by the client to initiate a connection
    :attr SERVER_HELLO:   server response to the hello
    :attr CHALLENGE_RESP: client response to finialize the connection
    :attr KEEP_ALIVE:     for packets automatically generated by a timeout
    :attr DISCONNECT:     client or server packet indicating graceful disconnect
    :attr APP:            user defined packet
    :attr APP_FRAGMENT:   user defined packet was split into fragments
...
class ConnectionBase(object):
    ...
    def _build_packet_impl(self, current_time: float, send_keep_alive: bool, resend_delay: float):
        ...
        current_msg_length = 0 # sum of length of messages in msgs, excluding overhead
        ...
        if self.pending_retry_msg:
            ...
            while idx < len(items):
                ...
                # calculate the size of the packet so far + this message
                size = len(msg.payload) + Packet.overhead(1+len(msgs)) + current_msg_length
                # if the message fits add it to the packet
                if size <= Packet.MAX_PAYLOAD_SIZE:
                    del self.pending_retry_msg[msgseq]
                    msgs.append(msg)
                    current_msg_length += len(msg.payload)
        ...
        while idx < len(self.outgoing_messages):
            ...
            # calculate the size of the packet so far + this message
            size = len(pending.payload) + Packet.overhead(1+len(msgs)) + current_msg_length
            # if the message fits add it to the packet
            if size <= Packet.MAX_PAYLOAD_SIZE:
                self.outgoing_messages.pop(idx)
                msgs.append(pending)
                current_msg_length += len(pending.payload)
            else:
                idx += 1
    ...
    def _recv_message(self, pkt_typ, msgseq, msg):
        ...
        try:
            ...
            #try:
            #    msgdat = Serializable.loadb(msg)
            #    name = str(type(msgdat))
            #except Exception as e:
            #    name = msg[:16]
            #print("drop duplicated typ=%s seq=%s (%s)" % (pkt_typ, msgseq, name))
            return
...
class ClientServerConnection(ConnectionBase):
    ...
    def _ClientHelloTimeout(self, success):
        if not success:
            mplogger.error("unable to connect to server")

    def _ChallengeResponseTimeout(self, success):
        if not success:
            # expecting a keep alive or other message
            mplogger.error("no response to challenge")
